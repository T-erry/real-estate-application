  ...
  return (
    <li className=" relative bg-white flex flex-col justify-between items-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow duration-150 m-[10px]">
      <Link  className="contents" to={`/category/${listing.type}/${id}`}>
        ...
        <Moment className="absolute top-2 left-2 bg-[#3377cc] tex-white uppercase text-xl font-semibold rounded-md px-2 py-1 shadow-lg" fromNow>{listing.timestamp?.toDate()}</Moment>
        <div className="w-full p-[10px]">
          ...
          <div className="flex items-center mt-[10px] space-x-10">
            <div className="flex items-center space-x-1">
      ...
      {onDelete && (
       <FaTrashAlt className="absolute bottom-2 right-2 h-[14px] cursor-pointer text-red-500" onClick={()=> onDelete(listing.id)}/> 
      )}
      {onEdit && (
